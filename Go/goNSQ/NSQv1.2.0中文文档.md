...
2. 一个`IDENTIFY`命令(以及载荷)和读/验证响应（参见[特征协商](https://nsq.io/clients/building_client_libraries.html#feature_negotiation)）
...
如果不支持特征协商（`nsqd` `v0.2.20+`引入）nsqd会作出`OK`回应，否则：
