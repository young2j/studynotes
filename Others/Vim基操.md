...
按`ESC`可进入该模式，该模式下可以使用快捷键命令，即可以按:输入命令行。
...
# 文件保存
:w filename  	保存文件内容至filename
v+:w filename 	保存部分文件内容至filename，v进入可视模式

# 文件合并，使用替换模式
:r + filename   将filename的内容提取至当前光标之后
:r !ls 			将当前目录下的文件名提取至当前光标之后，即提取外部命令输出

#在动作前输入数字会使动作重复多次。
0	 跳至行首(home)
$ 	 跳至行尾(end)
2w   光标向右移动2个单词位置至下一个单词起始处
3e	 光标向右移动3个单词位置至单词末尾

# 删除 d number motion
#使用操作符时输入数字可以使操作重复多次
dw 		移动光标至单词前=>删除光标之后的整个词语(delete word),等价于d1w
d$		删除光标到行尾的内容($表示end)
d2w  	删除光标处之后两个单词
dd  	等价于1dd,删除整行
2dd 	删除两行
...
u 		撤消最后执行的命令
U 		撤消对整行的修改
ctrl+R  撤消掉撤消命令,即恢复redo

# 复制
v+y		进入可视模式，移动光标选中文本按y进行复制
yw		复制一个单词

# 粘贴
p 		将删除的内容粘贴至光标之后，若删除了整行则在当前行的下方粘贴内容。

# 更改  c number motion
ce/cw 		进入编辑模式，输入内容替换当前光标后的整个单词(e单词末尾，w下一个单词起始处)
c$ 			改变当前光标至行末的所有内容，表现和d$一样


# 跳转定位
ctrl+g     "/tmp/tutorlrk0h2" [Modified] line 503 of 993 --50%-- col 1
gg			小写，定位至文件首行
G			大写，定位至文件末行
503G		行号+大写G，定位至指定行(小写不会跳转`)

# 搜索
/+str	 	搜索指定str，按n(next)跳至下一个匹配项，N返回上一个匹配项，正向搜索
?+str		搜索指定str，n和N代表的方向相反，反向搜索
/+str\c		搜索时，单次忽略大小写
:set ic		搜索时设置忽略大小写
:set noic
:set hls is	搜索时设置高亮显示
:set is 	搜索时部分匹配incsearch


# 替换
r+替换的内容  将当前光标后的一个字符替换为新的字符,r进入替换模式
R+替换的内容	 连续替换 

:s/old/new		替换当前行第一个匹配的串
:s/old/new/g 	替换当前行所有匹配的串
:#,#s/old/new/g 替换行号#至行号#之间所有匹配的串
:%s/old/new/g	全局替换匹配串
:%s/old/new/gc	全局替换匹配串(带提示)

# 执行外部shell
:!+shell

# 补全
:部分命令 + ctrl d 	查看补全项, tab补全
```

### 插入模式

在正常模式下，按`i、a、o`都可以进入插入模式，该模式下可以输入文本。

```shell
i   在当前光标位置处插入文本
a   光标移动到当前单词的末尾插入文本
A 	光标移动到当前行的末尾插入文本
o 	在当前光标处的下方新增空白行，进入编辑模式
O 	在当前光标处的上方新增空白行，进入编辑模式
```
