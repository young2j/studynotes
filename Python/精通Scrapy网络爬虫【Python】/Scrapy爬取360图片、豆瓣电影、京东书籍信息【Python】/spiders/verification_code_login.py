...
class VerificationCodeSpider(scrapy.Spider):
    ...
    def parse_login(self, response):  # 此方法既要完成登录有下载验证码图片
        ...
        if not login_response:  # 登录页响应,提取验证码图片的url
            VC_url = response.css(
                'label.field.prepend-icon img::attr(src)').extract_first()
            ...
            fmdata = {
                ...
                # response.body是图片的二进制数据
                'code': self.parse_VC_by_ocr(response.body)
            }
    ...
    def parse_login_ed(self, response):
        # 需要判断是否登录成功,不成功则重新登录
        # form请求的正文是json串,包含了用户验证的结果,转为python字典后根据error字段判断
